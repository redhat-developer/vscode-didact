...
export function processInputs(incoming : string, extensionPath : string) {
    let output : any[] = [];
    if (incoming) {
        ...
        if (query.text) {
            ...
            if (user) {
                handleUser(user, output, errorMessage);
            }
        }

        // now call the command
        callCommand(commandId, output, completionMessage, errorMessage);
    }
}
